...
// Confirm delete action when the delete button is clicked
function confirmDelete() {
    return confirm("Are you sure you want to delete this reservation?");
}

// Simple form validation or dynamic behavior for your project
document.getElementById('hotel').addEventListener('change', function() {
    const hotelValue = this.value;
    // Perform actions based on selected hotel (you can add specific logic here if needed)
});

// Ensure DOM is fully loaded before running script
document.addEventListener("DOMContentLoaded", function() {
    // Attach event listener to all delete buttons for confirmation
    ...
    // Attach event listener to the update form for submission confirmation
    const form = document.getElementById("update-form");
    form.addEventListener("submit", function(event) {
        // Confirmation before submission
        if (!confirm("Are you sure you want to update this reservation?")) {
            event.preventDefault(); // Cancel the submission if not confirmed
        }
    });
});